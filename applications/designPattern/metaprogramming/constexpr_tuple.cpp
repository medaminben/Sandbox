...
struct st_print_tuple {
    static void print(std::ostream& os, std::tuple<Types...> const& tpl) {
...
template<size_t index, typename... Types>
void print_tuple(std::ostream& os, std::tuple<Types...> const& tpl) {
...
// operator overload either with the struct template or with the function template 
// the reslt looks the same

// template<typename... Types>
// std:: ostream& operator<<(std::ostream& os, std::tuple<Types...> const& tpl) {
//     os << "{ "; 
...
constexpr void compile_time_loop(Operation operation, Types&&... args) {
...
void add(int a, double b) {
    std::cout  << "add("<<a<<","<<b<<") is called"<< " \n";
}
void product(int a, double b, size_t c) {
    std::cout  << "product(" << a <<","<<b<< "," << c << ") is called"<< " \n";
};
int main() { 
    ...
    std::cout  << "-----------------"           << " \n" <<
    "Count of t2 elements = " << tuple_t2_count << " \n" << 
    "Count of t3 elements = " << tuple_t3_count << " \n";
