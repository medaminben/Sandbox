...
bool is_prime_recur(size_t p) { 
    ...
    // (p < 4) ? (p > 1) : is_prime_recur_impl(p,p/2)
}
...
struct st_is_prime_impl {
    static const bool value = (p % d) 
            && st_is_prime_impl< p , d - 1 >::value;
};
// spezialisations cases from 0 to 3
template< size_t p> struct st_is_prime_impl<p,2> { 
    static const bool value = p % 2; 
};
template< size_t p> struct st_is_prime_impl<p,1> { 
    static const bool value = false; 
};
template< size_t p> struct st_is_prime_impl<p,0> { 
    static const bool value = false; 
};
// primary struct template
template<size_t p> 
...
///////////////////////////////////////////////////////////
int main() {
    // the maximum that is allowed by the class template 
    constexpr size_t limit = 600;
    std::cout << "------------------\n";
